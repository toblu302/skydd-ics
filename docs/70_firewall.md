...
Installationen kommer nu att försöka hitta rätt konfiguration för skärm och tangentbord där det bör gå att välja ***< Accept these Settings >*** genom att stega ner med piltangenten och tryck på *enter/retur*

![Skärm och tangentbordskonfiguration](images/installation_fw_3.png "Skärm och tangentbordskonfiguration")

I nästa meny så går det att välja vilken typ av installation som ska göras. Välj menyvalet *< Quick/Easy Install >* genom att trycka *enter/retur*

![Val av installationstyp](images/installation_fw_4.png "Val av installationstyp")

När valet är gjort så kommer en förfrågan som frågar om du verkligen är säker på ditt val. Här måste det accepteras att installation kommer vara automatisk och inga frågor kommer att ställas. Det innebär att den första hårdisken kommer att raderas och det går inte att ångra. Den säger även att det gåt att göra installationsförfarandet manuellt om större kontroll krävs.

Tryck *enter/retur* på *< OK >* när ok knappen är markerad för att förtsätta installationen.  

![Bekräfta gjorda installationsval](images/installation_fw_5.png "Bekräfta gjorda installationsval")
...
Det kommer att dyka upp en förfrågan efter en stund (fast det står i bekräftelserutan att inga frågor kommer ställas) där val av kärnmodul ska göras. Välj ***< Standard Kernel >*** genom att tryck på *enter/retur* när den är markerad.

![Val av operativsystemskärna](images/installation_fw_7.png "Val av operativsystemskärna")

Installatinen färdigställs och en sista meny kommer fram där den säger att datorn ska startas om, men att installationsmediet måste tas bort efter omstarten.

Välj ***< Reboot >*** genom att tryck på *enter/retur* när det valet är markerat.
...
### Konfigurera pfSense
När pfsense är installerat så behövs ytterliggare konfiguration för att brandväggen ska vara produktionsdugligt.
Den första startbilden som visas när datorn brandväggen startar úpp är en meny med flera möjliga val samt ip konfiguration på det nätverkskort som finns på datorn.

Om det finns minst två nätverkskort så kommer det ena vara konfigurerat för WAN, dvs utsidan på brandväggen. Det andra nätverkskortet kommer att vara insidan och det är den sidan där konfigurationen kan göras ifrån, via ett webbaserat gränssnitt.

![Uppstartsmeny](images/konfiguration_fw_1.png "Uppstartsmeny")  

Titta i konsollen vilket nätverkskort som har LAN och koppla in en dator på det nätverkskortet, datorn kommer då att få en ip adress dynamiskt i ip-spannet 192.168.1.2-254. För att logga in till gränssnittet så ska en webläsare öppnas och adressen ***http://192.168.1.1*** skrivas in.
...
Väl inloggad så kommer en konfigurationssguide starta. Klicka på ***Next*** i rutan.

![Installationsguide](images/konfiguration_fw_4.png "Installationsguide")

En informationsruta för det support avtal som kan tecknas visas. Klicka på ***Next*** i rutan.
...
Klicka på ***Next*** för att förtsätta.
...
![Tidssynkronosering](images/konfiguration_fw_7.png "Tidssynkronosering")
...
#### PPTP
  ...
  * *PPTP Local IP Address* Lokala ipadressen för PPTP
  * *PPTP Remote IP Address:* Fjärripadressen för PPTP
...
Klicka på *Click **here** to continue on to pfSense webConfiguratior* som pilen pekar på enligt bilden nedan för att komma till första sidan.
...
pfSense har en dashboard (statussida) som visas efter inlogging. Längst upp så finns det en meny där alla inställningsmöjligheter finns.
  ...
  * *VPN* menyn är till för att konfigurera VPNanslutningar
...
Efter installationen av pfSense så går det att konfigurera brandväggen direkt i konsollen och med en webbläsare samt med protokollet SSH. Konsolen och själva brandväggen måste ha fysiskt skydd mot angrepp. För att skydda mot hot via de andra nätverksbaserade åtkomstsätten behövs ytterliggare konfiguration.  

SSH är säkert redan efter installation men användare måste verifiera *fingeravtrycket* innan åtkomst till det sker över osäkra nätverk.

Det webbaserade gränsittet är åtkomligt över http på port 80 vilket är ett klartextprotokoll som inte bör användas efter initial konfiguration där konfigurationsdatorn är direktkopplad till brandväggen. Under det förfarandet så kan ett certifikat installeras för att asnluta krypterat med https.  
...
Som standard så tillåter pfSense inta att nätverkstrafik paserar WANnätvekrskortet. Det måste manuellt tillåtas.
...
Denny vy visar de regler som finns för WANporten. Andra nätverk så som LAN finns under en egen flik på samma menysida.

För att lägga till en ny brandväggsregel så går det att klicka på ***+-ikon-knappen*** som den röda pilen på bilden nedan pekar på.
...
En ny sida visas där en brandväggsregel kan skrivas med följande konfiguration
  ...
  * *TCP/IP Version* Vilken eller vilka ipversioner som regeln gäller för
    * *IPv4* gäller enbart IPv4 traffik
	* *IPv6* gäller enbart IPv6  traffik
	* *IPv4+IPv6* gäller både IPv4 och IPv6
  * *Protocol* Vilket eller vilka protokol regeln gäller för, valigtvis används främst
    * TCP
	* UDP
	* TCP och UDP
  * *Source* Inställning för begränsa eller negotiera avsändare
  ...
  * *Destination port range* är vilken eller vilka nätverksportar som ska tillåtas neligt regeln
...
När en regel har sparats så måste den även appliseras genom att klicka på knappen *Apply changes* som visas efter att regeln sparats.
