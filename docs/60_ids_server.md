...
intrång lyckats. IDS-servern används för att automatiskt undersöka all trafik på de nätverksinterface
som är aktiva, och den söker efter otillåten nätverkstrafik eller nätverkstrafik som uppvisar
misstänksamheter. 

Som regel är ett nätverkinterface aktiverat i samband med att servern installerats.
Detta nätverksinterface kan ändras, eller andra interface kan aktiveras med hjälp av systemverktyget 
[NetworkManager]. 

![Menyval för att slå på nätverksinterface](images/networkmanager_turnoff.png "Menyval för att slå på nätverksinterface")

Om IDS-systemet upptäcker trafik som den enligt uppsatt regelverk, så sparas information om detta i en 
databas. Dessa varningar kan sedan studeras, kommenteras och klassificeras med en webläsare.

![Detaljer för händelse](images/ids_alert_1.png "Snorby alert note")

## Installation av IDS server

Installationen är till största del automatiserad och enbart ett fåtal steg måste utföras för att börja använda IDS-servern.
...
Det räcker att ställa in *Link-Local Only* på det önskade nätverkskortet för att det ska fungera. Det bör inte sättas en IP-address på nätverkskortet då det är säkerhetssynpunkt är bra att maskinen inte är nåbar från andra nätverk.
...
Denna installation använder sig utav [Snort], [Barnyard2] och [Snorby]. Snorby är en webbapplikation där man kan studera
de rapporter som [Snort] har larmat på och sedan kommentera dessa.

Som standard är Snorby enbart tillgängligt på den lokala maskinen och inte över nätverket. På <http://127.0.0.1:81/> går det
att logga in med de kontonuppgifter enligt nedan som följer med en Snorby-installation. Första gången en användare använder gränssnittet via en webläsare kommer det ta en liten stund för programet att automatiskt färdigkonfigureras.

  * Användarnamn: **snorby@example.com** 
  * Lösenord: **snorby**

*OBS: Kontot måste givetvis ändras till ett unikt säkert lösenord som inte är känt av andra.*
...
Om siffrorna inte uppdateras kan det bero på att ett jobb har lagts upp för en uppdatering och den justerade tiden för uppdatring. Den problematiken går att lösa genom att gå till "Administration", "Worker & Job Queue", radera *SensorCache-jobbet* och sen lägga till ett sådant jobb igen.

### Vyer i Snorby

I snorbys översiktsvy går det se en mängd olika saker och man kan välja andra vyer att titta på insamlat 
data genom att välja olika flikar, exempelvis:

* *Signatures-fliken* visar vilka regler som träffat mest procentuellt
* *Sources-fliken* visar vilken IP-address det är som orsakat de flesta händelserna
* *Destinations-fliken* visar vilken IP-address det är som blir mest attackerad
* *Sensors-fliken* är mest intressant då flera sensorer används vilket vi inte beskriver i denna guide
* *Severities* och *Protocols* delar delar upp händelserna i allvarlighetsgrader och nätverksprotokoll

I nästa skärmbild så visas ett cirkeldiagram som beskiver fördelningen över hur de olika regler med signaturer 
har blivit träffade. Diagrammet beskriver i procent hur olika regler har matchat de nätverkspaket som inhämtats.

![Signaturträff-fördelning](images/ids_piechart_sigs.png "Snorby pie chart signatures")

Nästa skärmbild visar ett annat cirkeldiagram som beskriver fördelningen mellan de olika käll-IP-adresser som
förekommer bland den nätverkstrafik som uppfångats. Den här vyn ger snabbt en översikt över vilken eller vilka
som är de värsta IP-adresserna som är upphov till IDS-larm.

![Källa till händelser](images/ids_piechart_sources.png "Snorby pie chart sources")

Nedanstående lista är ett belysande exempel på hur Events-sidan kan se ut om attacker utförs mot **ICS/SCADA**-system.
Bland reglerna som larmar är att någon försökt skicka Modbus-trafik och DNP3-trafik in mot nätet: 

![IDS-regler som matchat attacker mot ICS/SCADA-system](images/ids_list_scada_rules_matching.png "Snorby ICS/SCADA händelser")

## Placering

För att vara verkningsfull så måste IDS-servern anslutas till ett nätverk. En IDS-server bör nättopologiskt 
placeras på platser i nätverk där det behövs insyn i vad som pågår i nätverket. 
Det vill säga, inkopplingen i nätet kommer att påverka vad sensor kan se och analysera. Inkoppling
måste ske efter viss eftertanke och planering. Det här avsnittet avhandlar detta.
...
automations- och processnätverk. Ytterliggare anslutningspunkter kan vara i nät som sitter i perimetern
...
exempelvis leda till att en mängd trafik är skräp eller attacker som inte är relevanta, inte avser att riktas mot organisationen, med mera.

Resultatet av IDS-systemets infånganade och analys beror förutom själva den nätverksmässiga anslutningen också
...
För att analysera nätverkstrafiken används [Snort] som är ett välkänt program för analys. Dess rapporter importeras sedan utav [Barnyard2] 
...
[SELinux] används för att låsa ner så att en sårbarhet i någon av de installerade komponenterna inte kan utnyttjas för att ta kontroll
över systemet.
...
I den här installation är **IDS/SCADA**-regler från <https://github.com/digitalbond/quickdraw/> installerade. De kan behövas justeras för att passa en specifik miljö. Reglerna använder bland annat variabler för servrar och klienter som bör sättas i **snort.conf** för att de ska larma på rätt händelser.
...
IDS-systemet kan vara anslutet mot nätverket på ett av flera sätt. Genom att sitta på port i en såkallad hub, där all trafik 
...
mer kapabla och avancerade switcharna brukar ha möjlighet att sätta en anslutningsport i så kallat
...
via en speciell anslutning kunna hämta ut trafikdata. Det finns flera fördelar med att använda tap:ar istället för 
att ansluta via switchportar, bland annat att man minskar risken att data går förlorad, att nätverkspaket inte
förvanskas eller påverkas av switch-utrustningen, med mera.
...
**/etc/snort/snort.conf**. Efter en omstart av Snort (dvs inaktiva/aktivera nätverkskortet i [NetworkManager]) ska de nya reglerna
...
Då måste kryptering och authenticering aktiveras så att den potentiellt känsliga trafiken inte kan ses
av någon obehörig. Snorbys användernamn och lösenord måste även de ändras för att förhindra att känslig
