#!/bin/sh
#
# Modified version of mysql_secure_installation, basically with hard 
# coded answers. Easier than running expect.
#
    ...
    while [ $status -eq 1 ]; do
        ...
        if [ "x$password" = "x" ]; then
            ...
            if [ ${first_time} -eq 0 ]; then
                if [ -f /root/.mariadb_rootpw ]; then
                    password=$(cat /root/.mariadb_rootpw)
                    hadpass=1
                fi
            fi
        else
...
set_root_password() {
    ...
    nytt_rootpass=$(cat /root/.mariadb_rootpw)
    esc_pass=`basic_single_escape "$nytt_rootpass"`
...
prepare
# below does not exist in the original either. wtf?
#find_mysql_client
set_echo_compat
